...
string XpertRequestResultHtmlExport::makeBodyString(const XpertRequestResult& _xpertRequestResult) const
{
    ...
    ss << "<body>" << endl
       ...
       << "        <h3> {{ clinical_data.clinical_data_translation }} </h3>" << endl
       << "{% if not exists(\"clinical_data.rows\") %}"
       << "            {{ clinical_data.none_translation }}" << endl
       << "{% else %}"
       ...
       << endl
          ...
          //       the title and the first covariate for nice page break. This div needs
          //       to be closed when there is no covariate or at the end of the first one.

       << "       <!-- Covariates -->" << endl
       << "       <div class='avoid-break-inside'>" << endl
       << "           <h3> {{ covariates.translation }} </h3>" << endl
       << "{% if not exists(\"covariates.rows\") %}"
       << "           {{ covariates.none_translation }}" << endl
       << "       </div>" << endl
       << "{% else %}"
       <<     "{% for covariate in covariates.rows %}"
       ...
       << endl
          ...
          // If there is no dosage time range
          //    Insert none
          // Else
       ...
       << "{% if not exists(\"treatment.rows\") %}"
       << "            {{ treatment.none_translation }}" << endl
       << "        </div>" << endl
       << "{% else %}"
       <<     "{% for dosage_time_range in treatment.rows %}"
       ...
       << endl
          ...
          //       "displayed adjustment" title in the first div we create. That is possible to achieve with
       ...
       << "{% endif %}"
          ...
          //                 Insert it
          //
          // Note: Here, the page breaking system is quiet simple. We just add a <div class='avoid-break-inside'>
