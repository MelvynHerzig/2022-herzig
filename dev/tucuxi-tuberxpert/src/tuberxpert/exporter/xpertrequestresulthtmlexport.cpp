...
//       << "        </table>" << endl
       ...
       //<< "        var adustmentsData = {{graph_data.adjustments}};" << endl                                      // Insert the js array of adjustments
      // << " {% for row in graph_data.rows %} {{ row.values }}  {% endfor %}"
       //<< "        var targets = {{graph_data.targets}};" << endl                                                 // Insert the js array of targets
       << "        var obj = new GraphFullData();" << endl
       << "        addTargets(obj, targets);" << endl                                                             // Insert the dates for X axis bounds.
    ...
    //getContactsJson(_xpertRequestResult.getGlobalResult().getAdminData(), json["contacts"]);
    getClinicalDataJson(_xpertRequestResult.getGlobalResult().getAdminData(), json["clinical_data"]);
    getCovariatesJson(_xpertRequestResult.getCovariateResults(), json["covariates"]);
    getTreatmentJson(_xpertRequestResult.getTreatment()->getDosageHistory(), json["treatment"]);
    getSamplesJson(_xpertRequestResult.getSampleResults(), json["samples"]);
    getAdjustmentJson(_xpertRequestResult.getAdjustmentData(), json["adjustments"]);
    ...
    return inja::render(ss.str(), json);
}
...
void XpertRequestResultHtmlExport::getGraphDataJson(const XpertRequestResult& _xpertRequestResult, inja::json& _graphDataJson) const
{
    ...
    // Create the js array of targets
    const vector<Core::TargetEvaluationResult>& targets = _xpertRequestResult.getAdjustmentData()->getAdjustments()[0].m_targetsEvaluation;

    stringstream targetsStream;
    targetsStream << "["; // Main array start

    // For each target in the best suggestion
    for (size_t t = 0; t < targets.size(); ++t) {

        // If not the first target, add a comma to separate the elements
        if (t != 0){
            targetsStream << ", ";
        }

        targetsStream << "["; // Inner target array start

        // Add target type
        targetsStream << "'" << Core::toString(targets[t].getTargetType()) << "'";

        // Add target min
        targetsStream << "," << targets[t].getTarget().getValueMin();

        // Add target best
        targetsStream << "," << targets[t].getTarget().getValueBest();

        // Add target max
        targetsStream << "," << targets[t].getTarget().getValueMax();


        targetsStream << "]"; // Inner target array end
    }

    targetsStream << "]"; // Main array end
    _graphDataJson["targets"] = targetsStream.str();
...
//    stringstream adjustmentsStream;
//    adjustmentsStream << "["; // Main array start

    // For each adjustment
    for (size_t a = 0; a < adjustments.size(); ++a) {

        // If not the first adjustment, add a comma to separate the elements
        if (a != 0){
//            adjustmentsStream << ", ";
        }

//        adjustmentsStream << "["; // Inner adjustment array start

        // For each cycle data
        for (size_t c = 0; adjustments[a].getData().size(); ++a) {

            const Core::CycleData& cycleData = adjustments[a].getData()[c];

            // If not the first cycle data, add a comma to separate the elements
            if (c != 0){
//                adjustmentsStream << ", ";
            }

//            adjustmentsStream << "["; // Inner cycle data array start

            // Add the cycle data start time
//            adjustmentsStream << "'" + Common::Utils::varToString(cycleData.m_start) << "'";

            // Add the cycle data offsets
//            adjustmentsStream << ", ";
//            adjustmentsStream << "["; // Inner time offsets array start
            ...
            inja::json test1;
            test1["values"] = timesStream.str();
            _graphDataJson["rows"].emplace_back(test1);

//            adjustmentsStream << "]"; // Inner time offsets array end


            // Add the cycle data values
//            adjustmentsStream << ", ";
//            adjustmentsStream << "["; // Inner values array start
            ...
            inja::json test2;
            test2["values"] = valuesStream.str();
            _graphDataJson["rows"].emplace_back(test2);



//            adjustmentsStream << "]"; // Inner values array end

//            adjustmentsStream << "]"; // Inner cycle data array end

        }

//        adjustmentsStream << "]"; // Inner adjustment array end
    }

//    adjustmentsStream << "]"; // Main array end

//    _graphDataJson["adjustments"] = adjustmentsStream.str();
