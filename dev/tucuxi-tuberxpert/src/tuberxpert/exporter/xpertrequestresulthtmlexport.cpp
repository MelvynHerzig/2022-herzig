...
void XpertRequestResultHtmlExport::getHeaderJson(XpertRequestResult& _xpertRequestResult, inja::json& _json) const
{
    LanguageManager& lm = LanguageManager::getInstance();

    // TuberXpert report translation
    _json["tuberxpert_report_translation"] = lm.translate("tuberxpert_report");
...
void XpertRequestResultHtmlExport::getDrugIntroJson(XpertRequestResult& _xpertRequestResult, inja::json& _json) const
{
    LanguageManager& lm = LanguageManager::getInstance();

    // Drug translation
    _json["drug_translation"] = lm.translate("drug");

    // Drug id translation
    _json["id_translation"] = lm.translate("id");

    // Last dose translation
    _json["last_dose_translation"] = lm.translate("last_dose");

    // Drug model translation
    _json["drug_model_translation"] = lm.translate("drug_model");

    // Drug id
    _json["drug_id"] = _xpertRequestResult.getXpertRequest().getDrugID();

    // Last dose
    _json["last_dose"] = varToString(_xpertRequestResult.getLastIntake()->getDose()) + " " + _xpertRequestResult.getLastIntake()->getUnit().toString();
