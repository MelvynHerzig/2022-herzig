...
string XpertRequestResultHtmlExport::makeBodyString(XpertRequestResult& _xpertRequestResult) const
{
    ...
    // Preparing the data to insert
    inja::json data;
    data["header"] = exportHeader(_xpertRequestResult);
    data["drugIntro"] = exportDrugIntro(_xpertRequestResult);
...
string XpertRequestResultHtmlExport::exportHeader(XpertRequestResult& _xpertRequestResult) const
{
    stringstream ss;

    // Making the header template
    ss << "        <table class='header'>" << endl
       << "            <tr>" << endl
       << "                <td>" << endl
       ...
       << "                </td>" << endl
       << "                <td>" << endl
       << "                    <b>Computed on:&nbsp;</b> 08.04.2022" << endl    // Insert computation time
       << "                </td>" << endl
       << "            </tr>" << endl
       << "        </table>" << endl;
    ...
    data["computationTime"] = dateStream.str();

    // Rendering
    return inja::render(ss.str(), data);
}

string XpertRequestResultHtmlExport::exportDrugIntro(XpertRequestResult& _xpertRequestResult) const
{
    stringstream ss;

    // Making the drug intro template
    ss << "        <h3>Drug</h3>" << endl
       << "        <table class='drug-intro bg-light-grey'>" << endl
       << "            <tr>" << endl
       << "                <th>Id</th>" << endl
       << "                <th>Last dose</th>" << endl
       << "                <th>Drug model</th>" << endl
       << "            </tr>" << endl
       << "            <tr>" << endl
       << "                <td>" << endl
       << "                    {{ drugId }}" << endl            // Insert drug id
       << "                </td>" << endl
       << "                <td>" << endl
       << "                    {{ lastDose }}" << endl          // Insert last dose
       << "                </td>" << endl
       << "                <td>" << endl
       << "                    {{ drugModelId }}" << endl       // Insert drug model id
       << "                </td>" << endl
       << "            </tr>" << endl
       << "        </table>" << endl;
    ...
    data["drugId"] = _xpertRequestResult.getXpertRequest().getDrugID();
    data["lastDose"] = varToString(_xpertRequestResult.getLastIntake()->getDose()) + " " + _xpertRequestResult.getLastIntake()->getUnit().toString();
    data["drugModelId"] =  _xpertRequestResult.getDrugModel()->getDrugModelId();
